...
//TODO: availebel_Rules = [required,integer,max:10,min:10,date,email];
/*
...
//TODO:AUTH Party
    ...
    public function getToken($reference)
    {
        ...
        $jwt = JWT::encode($payload, $_ENV['USER_KEY'], 'HS512');

        return $jwt;
    }

    public function getRefrechToken($reference)
    {
        $iat = time();
        $exp = $iat + 60 * 60 * 7 * 24;
        $payload = array(
    ...
    {
        $decoded = JWT::decode($token, $_ENV['USER_KEY'], array('HS512'));
        return $decoded;
    }
